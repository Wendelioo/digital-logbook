...
function ClassManagementDetail() {
  ...
  const handleRemoveStudent = async (studentId: number, classId: number) => {
    ...
    try {
      ...
      if (appBridge && typeof appBridge.RemoveStudentFromJoinClassByIDs === 'function') {
        await appBridge.RemoveStudentFromJoinClassByIDs(studentId, classId);
      } else {
        throw new Error('Class removal method is not available in this app build.');
      }
  ...
  const handleAddStudent = async () => {
    ...
    try {
      const appBridge = (window as any)?.go?.backend?.App;
      let available: ClassStudent[] = [];
      if (appBridge && typeof appBridge.GetAllStudentsForJoinClasses === 'function') {
        available = await appBridge.GetAllStudentsForJoinClasses(parseInt(id));
      } else {
        throw new Error('Student list method is not available in this app build.');
      }
  ...
  const handleAddStudentsToClass = async () => {
    ...
    try {
      ...
      const appBridge = (window as any)?.go?.backend?.App;
      if (appBridge && typeof appBridge.AddMultipleStudentsToJoinClass === 'function') {
        await appBridge.AddMultipleStudentsToJoinClass(studentIds, parseInt(id), user?.id || 0);
      } else {
        throw new Error('Add students method is not available in this app build.');
      }

      setShowAddModal(false);
      await loadClassDetails();
      toast(`Successfully added ${selectedStudents.size} student(s)!`, 'success');
    } catch (error) {
      console.error('Failed to add students:', error);
      const errorMessage = error instanceof Error ? error.message : 'Failed to add some students. Please try again.';
      if (errorMessage.toLowerCase().includes('department')) {
        toast('Add blocked: selected student department does not match this class department.', 'error');
      } else {
  ...
  const studentTableColumnCount = isEditMode ? 6 : 5;
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm max-w-7xl mx-auto my-2 sm:my-4 p-4 sm:p-6 relative">
          ...
          <div className="overflow-x-hidden overflow-y-auto">
            <table className="w-full table-fixed">
              <thead>
                <tr>
                  <th colSpan={studentTableColumnCount} className="px-4 py-2 text-left border-b border-gray-200 bg-gray-50">
              ...
              <tbody className="bg-white text-sm">
                <tr>
                  ...
                  <td className="px-4 py-2 text-gray-900" colSpan={studentTableColumnCount - 3}>{classInfo.schedule || 'N/A'}</td>
                </tr>
                <tr>
                  ...
                  <td className="px-4 py-2 text-gray-900" colSpan={studentTableColumnCount - 3}>{classInfo.room || 'N/A'}</td>
                </tr>
                <tr>
                  <td className="px-4 py-2 font-semibold text-gray-700 whitespace-nowrap">EDP Code:</td>
                  <td className="px-4 py-2 text-gray-900">{classInfo.edp_code || 'N/A'}</td>
                  <td className="px-4 py-2 font-semibold text-gray-700 whitespace-nowrap">Join Code:</td>
                  <td className="px-4 py-2 text-gray-900 font-medium" colSpan={studentTableColumnCount - 3}>{classInfo.join_code || 'N/A'}</td>
                </tr>
                <tr>
                  ...
                  <td className="px-4 py-2 text-gray-900" colSpan={studentTableColumnCount - 3}>{classInfo.school_year || 'N/A'}</td>
                </tr>
                <tr>
                  <td className="px-4 py-2 font-semibold text-gray-700 whitespace-nowrap">Instructor:</td>
                  <td className="px-4 py-2 text-gray-900" colSpan={studentTableColumnCount - 1}>{classInfo.teacher_name || 'N/A'}</td>
                </tr>
              </tbody>

              <thead>
                <tr>
                  <th colSpan={studentTableColumnCount} className="px-4 py-3 text-left border-b border-gray-200 bg-gray-50">
                ...
                <tr className="bg-gray-100">
                  <th className={`${isEditMode ? 'w-[8%]' : 'w-[10%]'} px-2 py-2 text-center text-xs font-bold text-gray-700 uppercase whitespace-nowrap`}>No.</th>
                  <th className={`${isEditMode ? 'w-[16%]' : 'w-[18%]'} px-2 py-2 text-left text-xs font-bold text-gray-700 uppercase whitespace-nowrap`}>Student ID</th>
                  <th className={`${isEditMode ? 'w-[24%]' : 'w-[24%]'} px-2 py-2 text-left text-xs font-bold text-gray-700 uppercase whitespace-nowrap`}>Name</th>
                  <th className={`${isEditMode ? 'w-[28%]' : 'w-[30%]'} px-2 py-2 text-left text-xs font-bold text-gray-700 uppercase whitespace-nowrap`}>Email</th>
                  <th className={`${isEditMode ? 'w-[16%]' : 'w-[18%]'} px-2 py-2 text-left text-xs font-bold text-gray-700 uppercase whitespace-nowrap`}>Contact</th>
                  {isEditMode && (
                    <th className="w-[8%] px-2 py-2 text-center text-xs font-bold text-gray-700 uppercase whitespace-nowrap">Actions</th>
                  )}
                </tr>
              </thead>
                ...
                {filteredStudents.length > 0 ? (
                  filteredStudents.map((student, index) => (
                    <tr key={student.student_user_id} className="hover:bg-gray-50 border-b border-gray-100">
                      ...
                      <td className="px-2 py-1.5 text-gray-900 whitespace-normal break-words" style={{ wordBreak: 'break-word', overflowWrap: 'anywhere' }}>
                        {student.last_name}, {student.first_name} {student.middle_name ? student.middle_name.charAt(0) + '.' : ''}
                      </td>
                      <td className="px-2 py-1.5 text-gray-700 whitespace-normal break-words" style={{ wordBreak: 'break-word', overflowWrap: 'anywhere' }}>
                        {student.email || '—'}
                      </td>
                      <td className="px-2 py-1.5 text-gray-700 whitespace-normal break-words" style={{ wordBreak: 'break-word', overflowWrap: 'anywhere' }}>
                  ...
                  <tr>
                    <td colSpan={studentTableColumnCount} className="px-6 py-8 text-center">
      ...
      {showAddModal && (
        ...
        >
          <div className="modal-surface w-full max-w-2xl mx-2 sm:mx-4 relative max-h-[calc(100vh-2rem)] flex flex-col overflow-hidden">
            ...
            <div className="text-center p-4 sm:p-6 pb-3 sm:pb-4 flex-shrink-0">
              <h2 className="text-2xl font-bold text-blue-600 mb-2">Add</h2>
              <div className="w-24 h-0.5 bg-blue-600 mx-auto"></div>
              <p className="text-gray-600 mt-4">Select students to add in {classInfo?.subject_code}</p>
            </div>

            <div className="px-4 sm:px-6 pb-4 sm:pb-6 flex-1 overflow-hidden flex flex-col">
                ...
                {filteredAvailableStudents.length > 0 ? (
                  ...
                  <div className="px-6 py-12 text-center">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      {searchTerm ? 'No students match your search criteria.' : 'All students are already in class or there are no students in the system.'}
                    </p>
                  </div>
                )}
                </div>
