...
function Reports() {
  ...
  return (
    <div>
      ...
      >
        {pendingReports.length === 0 ? (
          <div className="min-h-[180px] flex items-center justify-center text-center text-sm text-gray-600">
                ...
                <div key={report.id} className="py-3 flex items-start justify-between gap-3">
                  ...
                  <div className="flex flex-col gap-1 items-end">
                    ...
                    <button
                      ...
                      className="inline-flex h-7 w-7 items-center justify-center text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-full hover:bg-emerald-100 transition-colors"
                      title="Confirm resolved"
                      aria-label="Confirm resolved"
                    >
        ...
        <div className="overflow-x-auto">
            <table className="w-full divide-y divide-gray-200 table-fixed">
              ...
              <thead className="bg-gray-50 sticky top-0 z-10">
                <tr>
                  <th className="pl-4 pr-3 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">User</th>
        ...
        {selectedReport && (() => {
          ...
          return (
            <div className="space-y-5">
              ...
              {/* Comments */}
              {(() => {
                const userComment = getOptionalUserComment(selectedReport.additional_comments);
                if (!userComment) return null;

                return (
                <div>
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Additional Comments</p>
                  <p className="text-sm text-gray-700 bg-gray-50 rounded-lg p-3 whitespace-pre-wrap">{userComment}</p>
                </div>
                );
              })()}

              {/* Working Student Notes */}
              {selectedReport.forward_notes && (
                <div>
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Forward Notes</p>
                  <p className="text-sm text-gray-700 bg-amber-50 border border-amber-100 rounded-lg p-3 whitespace-pre-wrap">{selectedReport.forward_notes}</p>
                </div>
              )}
