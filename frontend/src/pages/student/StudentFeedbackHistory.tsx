...
function parseEquipmentIssues(comments: string | null | undefined): Record<string, string> {
  ...
  const parts = comments.split(/;\s*/);
  for (const part of parts) {
    const match = part.match(/^(Computer|Monitor|Keyboard|Mouse|Additional):\s*(.+)$/i);
...
function DetailsModal({ feedback, onClose }: { feedback: Feedback; onClose: () => void }) {
  ...
  const issueTexts = parseEquipmentIssues(feedback.comments);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-5 space-y-4 overflow-y-auto">
          ...
          {/* Additional comments */}
          {issueTexts['additional'] && (
            <div>
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Additional Comments</p>
              <p className="text-sm text-gray-700 whitespace-pre-wrap">{issueTexts['additional']}</p>
            </div>
          )}
        </div>
